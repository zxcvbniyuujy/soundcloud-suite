...
(async () => {
  ...
  // the hub on every tab, over the shade
  await ensureHub(true); await sleep(7000); await shade(true); await sleep(300);
  ...
  await tab('lyrics'); await sleep(400); await shade(false);
  ...
  await step('shuffle', async () => {
    ...
    console.log('shuffle button', JSON.stringify(btn));
    await ensureHub(true); await sleep(2500); await shade(true); await sleep(300);
    await tab('queue'); await sleep(800); await shot('hub-queue-shuffle', padRect(await panelRect()));
    await shade(false); await ensureHub(false);
  });
