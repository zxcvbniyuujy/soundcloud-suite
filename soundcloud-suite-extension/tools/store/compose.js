...
// captures in store/raw. One system: a brand-orange column carries the message, the real UI sits in the light column
// at 1:1 CSS pixels (so it is pixel-crisp on 2× displays), and one element per shot may cross the seam.
...
const card = (name, radius = 22, extra = '') => { const r = rects[name]; const pl = r.pl == null ? PAD : r.pl, pt = r.pt == null ? PAD : r.pt; const w = r.w || r.width - PAD * 2, h = r.h || r.height - PAD * 2; return `<div class="card" style="width:${w}px;height:${h}px;border-radius:${radius}px;${extra}"><img src="${dataUri(name)}" style="width:${r.width}px;height:${r.height}px;margin:-${pt}px 0 0 -${pl}px"></div>`; };
// a region of a full-page capture, by its CSS-pixel rect, enlarged `s` times: the zoomed callout
const cutout = (name, x, y, w, h, s, extra = '') => `<div class="cut" style="width:${Math.round(w * s)}px;height:${Math.round(h * s)}px;background:url(${dataUri(name)}) -${Math.round(x * s)}px -${Math.round(y * s)}px / ${1280 * s}px ${800 * s}px no-repeat;${extra}"></div>`;
...
.stage { position: relative; overflow: hidden; background: radial-gradient(90% 80% at 72% 28%, #FFFFFF 0%, #F4F2EF 100%); }
.band { position: absolute; left: 0; top: 0; bottom: 0; background: linear-gradient(162deg, #FF6A1C 0%, ${ORANGE} 46%, #F04C00 100%); }
...
h1 { font-size: 60px; line-height: 1.02; font-weight: 800; letter-spacing: -.035em; max-width: 450px; }
...
.card { position: absolute; overflow: hidden; box-shadow: 0 40px 90px rgba(25,12,0,.40), 0 8px 24px rgba(25,12,0,.22); }
.frame { position: absolute; border-radius: 14px; overflow: hidden; box-shadow: 0 40px 90px rgba(25,12,0,.40), 0 8px 24px rgba(25,12,0,.22); background: #111; }
...
.strip { position: absolute; border-radius: 16px; overflow: hidden; box-shadow: 0 30px 60px rgba(25,12,0,.38); }
.strip img { display: block; }
.cut { position: absolute; border-radius: 16px; box-shadow: 0 0 0 3px #fff, 0 26px 50px rgba(25,12,0,.38); }
`;
...
// 20 px off the seam and the edge), and a card can bleed off the bottom on purpose but never off the right
const COL_X = 560, COL_W = 720;
const centred = (w) => COL_X + Math.round((COL_W - w) / 2);
const cardW = (name) => rects[name].w || rects[name].width - PAD * 2, cardH = (name) => rects[name].h || rects[name].height - PAD * 2;
const centredCard = (name, top) => card(name, 22, `left:${centred(cardW(name))}px;top:${top}px`);
const shuffleUi = () => {
  // the likes page in a frame, the hub's Queue tab over its lower right, and the Shuffle Play button's queued count
  // enlarged in a callout that crosses the seam into the copy column: the one thing this shot has to make legible
  const fw = 680, fx = COL_X + 20, fy = 60;
  const qw = Math.round(cardW('hub-queue-shuffle') * 0.86);
  const b = rects['shuffle-btn'] || { x: 272, y: 190, width: 110, height: 32 };
  // the cut stays inside the tab row's own background: 6 px above and below the button, and short of the menu icon after it
  const cx = Math.round(b.x - 98), cy = Math.round(b.y - 6), cw = Math.round(b.width + 104), ch = b.height + 12, s = 1.6;
  return frame('page-shuffle', fw, `left:${fx}px;top:${fy}px`)
    + card('hub-queue-shuffle', 22, `left:${COL_X + COL_W - qw - 36}px;top:${fy + 180}px;transform:scale(.86);transform-origin:top left`)
    + cutout('page-shuffle', cx, cy, cw, ch, s, `left:${COL_X - 104}px;top:562px`);
};
const toolsUi = () => {
  // the command palette at 1:1 and, below it, the right end of the player bar with the suite's own buttons
  const top = 78, sw = 584, bar = rects.bar;
  return centredCard('cmdk', top) + `<div class="strip" style="left:${centred(sw)}px;top:${top + cardH('cmdk') + 44}px;width:${sw}px;height:${bar.height}px"><img src="${dataUri('bar')}" style="width:${bar.width}px;height:${bar.height}px;margin-left:-${bar.width - sw}px"></div>`;
};
const shots = [
  ...
  { file: 'screenshot-5-tools', eyebrow: 'Player tools', h1: 'Every tool in the bar', sub: 'Speed, A–B loop, sleep timer, track info, a floating lyric line, and a Ctrl+K palette that reaches every setting.', ui: toolsUi },
];
...
const tile440 = () => `<html><head><style>${CSS} .t { width:440px;height:280px;background:linear-gradient(162deg, #FF6A1C 0%, ${ORANGE} 46%, #F04C00 100%);position:relative;overflow:hidden;color:#fff } .t h1 { font-size:34px;letter-spacing:-.03em;position:absolute;left:36px;top:132px } .t .tag { position:absolute;left:36px;top:190px;font-size:15px;font-weight:600;opacity:.9;letter-spacing:.01em } .t .m { position:absolute;left:36px;top:44px }</style></head><body><div class="t"><div class="m">${markSvg(150, '#fff', ORANGE, '#fff')}</div><h1>SoundCloud Suite</h1><div class="tag">Lyrics · Audio · Shuffle · Themes</div></div></body></html>`;
const marquee1400 = () => `<html><head><style>${CSS} .stage { background:linear-gradient(162deg, #FF6A1C 0%, ${ORANGE} 46%, #F04C00 100%) } .mq h1 { font-size:64px;max-width:560px } .mq .sub { font-size:22px;max-width:520px }</style></head><body><div class="stage mq" style="width:1400px;height:560px"><div class="copy" style="top:104px"><div style="margin-bottom:34px">${markSvg(170, '#fff', ORANGE, '#fff')}</div><h1>SoundCloud Suite</h1><div class="sub">Synced lyrics, studio-grade audio, full-library shuffle and a cleaner SoundCloud.</div></div>${card('hub-audio', 22, 'left:704px;top:36px;transform:scale(.78);transform-origin:top left')}${card('hub-lyrics', 22, `left:${1400 - 40 - Math.round(cardW('hub-lyrics') * 0.78)}px;top:96px;transform:scale(.78);transform-origin:top left`)}</div></body></html>`;
(async () => {
